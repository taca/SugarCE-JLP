...
$mod_strings = array(
	...
	'LBL_HIDEOPTIONS' => '非表示' /*for 508 compliance fix*/,
...
'help'=>array(
	...
	'module'=>array(
		...
		'audit'=>'このチェックボックスをチェックすることで変更履歴の対象になります。特定のフィールドの変更履歴を保管し、管理者が閲覧できるようになります。',
		...
		'type_basic'=>'<b>ベーシック（Basic）</b>テンプレートタイプでは、名前、担当ユーザ、作成日付、詳細などの基本的なフィールドが用意されています。',
		...
		'type_issue'=>'<b>問合せ（Issue）</b>テンプレートタイプでは、番号、ステータス、優先度、詳細など、問合せや不具合管理に必要なフィールドが用意されています。<br/><br/>標準の問合せや不具合管理に似たモジュールを作成する場合に利用してください。',
	...
	'dropdowns'=>array(
		...
		'editdropdown'=>'ドロップダウンリストはどのモジュールの標準もしくはカスタマイズされたドロップダウンフィールドにも利用可能です。<br><br>ドロップダウンリストの<b>名前</b>をご指定ください。<br><br>アプリケーションに言語パックがインストールされている場合は、リストアイテムから<b>言語</b>を選択してください。<br><br><b>キー</b>フィールドで、ドロップダウンリスト内の値を指定してください。この名前はユーザに見えるドロップダウンリストには現れません。<br><br><b>表示ラベル</b>フィールドで、ラベルを指定してください。このラベルはユーザから見えます。<br><br>アイテム名と表示ラベルを指定後、<b>追加</b>をクリックしてドロップダウンにアイテムを追加してください。<br><br>リスト中のアイテムの順番を変えるには、持っていきたい位置にアイテムをドラッグアンドドロップしてください。<br><br>アイテムのラベルを編集するには、<b>編集アイコン</b>をクリックし、新しいラベルを入力してください。ドロップダウンからアイテムを削除するには、<b>削除アイコン</b>をクリックしてください。<br><br>表示ラベルへの変更を元に戻すには、<b>元に戻す</b>をクリックしてください。元に戻したものを再実行するには、<b>再実行する</b>をクリックしてください。<br><br><b>保存</b>をクリックし、ドロップダウンを保存してください。',
	...
	'subPanelEditor'=>array(
	    ...
	    'historyDefault'=> '<b>初期状態に戻す</b> をクリックするとシステムの初期状態のレイアウトに戻すことができます。',
	...
	'listViewEditor'=>array(
		...
		'historyBtn'=> '<b>履歴の表示</b>をクリックし、 以前に保存された履歴を閲覧およびリストアします。',
		'historyDefault'=> '<b>初期状態に戻す</b> をクリックするとシステムの初期状態のレイアウトに戻すことができます。',
		'Hidden' 	=> '<b>非公開</b>フィールドは一覧画面ではユーザに表示されません。',
		'Available' => '<b>利用可能</b>フィールドはデフォルトでは画面上に表示されませんが、ユーザによって追加可能なフィールドとなります。',
		'Default'	=> '<b>デフォルト</b>フィールドは一覧表示画面上でユーザに表示されるフィールドとなります。（ただし一覧表示画面がカスタマイズされていない場合に限ります。）'
	),
	'popupListViewEditor'=>array(
		'modify'	=> 'すべてのフィールドを <b>一覧画面</b> に表示できます。<br><br> <b>デフォルト</b> は一覧画面を表示した際表示されるフィールドとなります。<br/><br/> <b>非表示</b> はデフォルトまたは利用可能に追加することができます。',
		'savebtn'	=> '<b>保存して配置</b>をクリックし、変更を保存しそれらをモジュール内で有効化します。',
		'historyBtn'=> '<b>履歴の表示</b>をクリックし、以前に保存された履歴を閲覧およびリストアします',
		'historyDefault'=> '<b>初期状態に戻す</b> をクリックするとシステムの初期状態のレイアウトに戻すことができます。',
		'Hidden' 	=> '<b>非表示</b>フィールドは一覧画面に表示されません。',
		'Default'	=> '<b>デフォルト</b> フィールドは一覧に表示されます。また、ユーザによってカスタマイズできません。'
	),
	'searchViewEditor'=>array(
		...
		'Hidden' 	=> '<b>非表示</b>フィールドは検索ビューに表示されないフィールドです。',
		'historyBtn'=> '<b>履歴の表示</b>をクリックし、 以前に保存された履歴を閲覧およびリストアします。',
		'historyDefault'=> '<b>初期状態に戻す</b> をクリックするとシステムの初期状態のレイアウトに戻すことができます。',
	...
	'studioWizard'=>array(
	    ...
	    'popupHelp' =>'<b>ポップアップ</b>の画面のカスタマイズ内容が表示されます。<br>',
	    'PopupListViewBtn' =>'<b>ポップアップ 一覧</b> はポップアップ検索画面の一覧画面です。',
	    'PopupSearchViewBtn' =>'<b>ポップアップ 検索</b> はポップアップ一覧画面を表示するための検索条件です。',
		...
		'convertLeadHelp' => "ここでは、レイアウト画面にモジュールを追加し、既存のレイアウトを変更することができます。<br/>
		表のカラムをドラッグして変更することができます。<br/><br/>

		<b>モジュール:</b> モジュール名<br/><br/>
		<b>必須:</b> コンバートする再に関連を選択または新規作成が必須とします。<br/><br/>
		<b>データコピー:</b> 新しく作成さするときに同じ名前のフィールドがあればリードのフィールドの内容をコピーします。<br/><br/>
		<b>選択を許可:</b> コンバート中に取引先担当者への関連モジュールを選択できるようにします。<br/><br/>
		<b>編集:</b> モジュールのコンバートレイアウトを変更します。<br/><br/>
		<b>削除:</b> コンバートからモジュールを削除します。<br/><br/>
		",
...
'assistantHelp'=>array(
	...
	'module'=>array(
		...
		'create'=>'作成するモジュールの<b>タイプ</b>の値を選択する場合、そのモジュールに必要なフィールドを想定してください。<br/><br/>それぞれのモジュールテンプレートは、タイトルで表されるようなタイプのモジュールに必要なフィールドをあらかじめ用意しています。<br/><br/><b>基本（Basic）</b> - 標準モジュールで使われている名前、担当ユーザ、チーム、作成日、詳細などのフィールドを用意しています。<br/><br/> <b>会社（Company）</b> - 標準の取引先モジュールのように、組織、会社で使われる会社名、産業、請求先住所などのフィールドを用意しています。<br/><br/> <b>個人（Person）</b> - 標準の取引先担当者やリードモジュールのように、敬称、職位、名前、住所、電話番号など、個人に必要なフィールドを用意しています。<br/><br/><b>課題（Issue）</b> - 問合せ管理やバグ管理に必要な番号、ステータス、優先度、詳細といったフィールドを用意しています。<br/><br/>注意：モジュールの作成後、テンプレートが提供するフィールドをカスタマイズしたり、必要なフィールドをさらに追加することができます。',
...
'LBL_HOME' => 'ホーム',
'LBL_NONE'=>'-なし-',
'LBL_DEPLOYE_COMPLETE'=>'配置が完了しました。',
'LBL_DEPLOY_FAILED'   =>'エラー：配置プロセス中にエラーが発生しました。正常にインストールできていない可能性があります。',
...
'LBL_POPUP'=>'ポップアップ画面',
'LBL_POPUPLIST'=>'ポップアップ一覧',
'LBL_POPUPLISTVIEW'=>'ポップアップ一覧画面',
'LBL_POPUPSEARCH'=>'ポップアップ検索画面',
...
'LBL_TAB_PANELS_HELP' => '各パネルをタブで切り替えられるように表示します。',
'LBL_DROPDOWN_TITLE_NAME' => 'ドロップダウン名',
'LBL_DROPDOWN_LANGUAGE' => 'ドロップダウンの言語',
'LBL_DROPDOWN_ITEMS' => 'ドロップダウン項目',
'LBL_DROPDOWN_ITEM_NAME' => 'キー',
'LBL_DROPDOWN_ITEM_LABEL' => '表示ラベル',
'LBL_SYNC_TO_DETAILVIEW' => '詳細画面と同期',
'LBL_SYNC_TO_DETAILVIEW_HELP' => '詳細画面と編集画面のフィールドを同期したい場合にはこのオプションを選択してください。 <br>詳細画面にフィールドを配置したい場合には編集画面のレイアウトを変更し保存または保存して配置をクリックすると自動的に展開されます。<br>よって詳細画面だけを変更することはできなくなります。',
'LBL_SYNC_TO_DETAILVIEW_NOTICE' => 'この詳細画面は編集画面と同期しています。<br> 詳細画面のフィールドの配置は編集画面のフィールドの配置を反映しています。<br>よってこのページでは詳細画面を変更することができません。詳細画面のみ編集したい場合には編集画面のレイアウトで同期を解除してください。 ',
'LBL_COPY_FROM_EDITVIEW' => '編集画面からコピー',
'LBL_DROPDOWN_BLANK_WARNING' => '値は、キーと表示ラベルの両方に必要です。空白の項目を追加するには、キーと表示ラベルに任意の値を入力せずに[追加]をクリックします。',
...
'LBL_QUESTION_POPUP' => 'ポップアップレイアウトの編集を選択',
//CUSTOM FIELDS
'LBL_RELATE_TO'=>'関連先',
'LBL_NAME'=>'名前',
'LBL_LABELS'=>'ラベル',
...
'LBL_SECTION_VISIBILITY_EDITOR'=>'表示/非表示の編集',
//WIZARDS
...
'ERROR_MINIMUM_FIELDS' => 'レイアウトには、少なくとも1つのフィールドが含まれている必要があります。',
'ERROR_GENERIC_TITLE' => 'エラー：エラーが発生しました。',
'ERROR_REQUIRED_FIELDS' => '以下の必須フィールドがレイアウトに配置されていませんがよろしいでしょうか？:  ',
'ERROR_ARE_YOU_SURE' => 'この操作を続けますか？',
...
'LBL_JS_REMOVE_MODULE' => 'このモジュールを削除しますか？一度削除すると元に戻すことができません。',
'LBL_JS_DEPLOY_PACKAGE' => '再配置すると、スタジオで行った変更が上書きされます。この操作を続行しますか？',

'LBL_DEPLOY_IN_PROGRESS' => 'パッケージの配置',
'LBL_JS_VALIDATE_NAME'=>'パッケージ名 - 空白を含まず英字から始まる英数字である必要があります',
'LBL_JS_VALIDATE_PACKAGE_NAME'=>'そのパッケージ名はすでに作成されています。',
...
//CONFIRM
'LBL_CONFIRM_FIELD_DELETE'=>'カスタムフィールドを削除すると関連付けられたデータも全て削除されます。フィールドをすべてのレイアウト上から削除する必要があります。続行しますか？',
...
'LBL_CONFIRM_LOWER_LENGTH' => 'データが削除され元に戻すことができなくなります。操作を続行しますか？',
...
'LBL_POPHELP_IMAGE_WIDTH'=>'数値(ピクセル)で幅を指定してください。<br> アップロードされた画像はこの幅で表示されます。',
'LBL_POPHELP_IMAGE_HEIGHT'=>'数値(ピクセル)で高さを指定してください。<br> アップロードされた画像はこの幅で表示されます。',
...
'LBL_ILLEGAL_FIELD_VALUE' =>"ドロップダウンのキーに引用符を利用することはできません。",
'LBL_CONFIRM_SAVE_DROPDOWN' =>"ドロップダウンリストからアイテムを削除します。削除されるとこの値を利用しているフィールドがあった場合には値が表示されなくなります。操作を続行しますか？",
